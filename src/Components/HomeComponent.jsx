      ...
      case 'A': {
        if (!this.state.dispatching) {
          this.setState({ dispatching: true }, () => {
            ...
            setTimeout(() => this.setState({ dispatching: false }), 1500);
          });
        }
        break;
      }
  ...
  render() {
    ...
    return (
      <div className={this.props.visibility ? '' : 'hidden'}>
        <h3>Bon Voyage</h3>
        <p><span
          className="text-warning">The Sector is at war. Greed and corruption split the Confederation in half.<br/>
                    Outlaws attack everybody on sight. For many of them there is a price on their heads.
                    <br/>Two enemy civilizations, <span className="text-info">The Scourge</span> and <span
            className="text-info">Quadrant 12</span>
        ...
        <div className="text-center">
          <button id='btn-go-to-ship-selection' onClick={(e) => this.goToShipSelection(e)}
                  className="action-red">» CREATE EXPEDITION FLEET
          </button>
        </div>
